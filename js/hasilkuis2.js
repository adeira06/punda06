...
if(score >= 50) {
    ...
    kett.innerHTML = 'Selamat! Anda Telah Menyelesaikan Seluruh Materi pada Penyusunan Data.'
...
if(localStorage && localStorage.getItem('kuis2')) {
    quizResults = JSON.parse(localStorage.getItem('kuis2'))

    if(result.correctAnswer != "" && result.wrongAnswer != "") {
        quizResults.results.push(result)
    }

    localStorage.setItem('kuis2', JSON.stringify(quizResults))
    clearResults()
} else {
    localStorage.setItem('kuis2', JSON.stringify(quizResults))
    if(result.correctAnswer != "" && result.wrongAnswer != "") {
